...
const MyBooking = () => {
    ...
    const deleteFromHistory = (id) => {
        ...
        const hidden = JSON.parse(localStorage.getItem("hiddenBookings") || "[]");
        localStorage.setItem("hiddenBookings", JSON.stringify([...hidden, id]));
    ...
    const getAverageRating = (booking, providerId) => {
        const ratings = booking.ratings?.filter(
                ...
                <div key={b._id} className="booking-card">
                        ...
                        {b.providers?.map(p => (
                            <div key={p._id} className="provider-info">
                                ...
                                {b.ratings?.some(r => r.providerId === p._id) && (
                                    <p>
                                        Average Rating:
                                        {"★".repeat(Math.round(getAverageRating(b, p._id)))}
                                        {"☆".repeat(5 - Math.round(getAverageRating(b, p._id)))}
                                        ({getAverageRating(b, p._id)})
                                    </p>
                                )}
                    ...
                    {(b.status === "completed" || b.status === "cancelled") && (
                        <button
                            ...
                            onClick={() => deleteFromHistory(b._id)}
                        >
                            Delete
                        </button>
                    )}
